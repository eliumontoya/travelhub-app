...
The MCP server MUST expose its registered tools through the protocol's native tool-listing mechanism (`listTools`), so a connected client can discover the available tool set without out-of-band knowledge.
...
- THEN the response lists exactly the registered service-document tools
