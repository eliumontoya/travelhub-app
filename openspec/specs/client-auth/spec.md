...
The system MUST expose `/client/login` accepting email and PIN. On valid credentials it MUST issue a signed `HttpOnly` session cookie. On invalid credentials it MUST NOT issue a cookie and MUST return a generic authentication error.
...
- AND the client is redirected to a post-login surface
