...
- [ ] 7.3 **GREEN** — Create `src/app/dashboard/settings/accounts/page.tsx` (Server Component): `await requireAdmin()` (non-admins redirect to the dashboard home route), `const profiles = await listProfiles()`, render `<FeatureManagerClient profiles={profiles} />`. Not gated by the `settings` feature — admin-only by role.
...
- [ ] 7.5 **GREEN** — Modify `src/app/dashboard/layout.tsx` (second edit, ships with this slice): add an admin-only "Cuentas" link to the accounts route, rendered when `account?.role === "admin"` (outside the six-feature catalog — account management edits the flags, it is not a flag).
- [ ] 7.6 **Verify** — `npx vitest run src/app/dashboard/settings/accounts/__tests__/actions.test.ts` passes; `npx tsc --noEmit` passes; runtime harness (mock): `npm run dev` as `mock-admin` → open the accounts route → toggle `mock-agent` features → confirm next-request nav reflects the change; as `mock-agent` → a GET to the accounts route redirects to the dashboard home route.
